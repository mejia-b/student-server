    ...
    function readNextFile() {
      ...
      fs.readFile(fname, "utf8", (err, data) => {
        if (err) {
          ...
          arr.push(JSON.parse(data));
          readNextFile(); // Continue reading the next file
        }
      });
    }
 ...
 */
app.get('/students/lastname/:lastname', function (req, res) {
  var lastname = req.params.lastname;

  // read all files and store into an array
  filesread = 0;
  var studentList = [];
  ...
  glob("students/*.json", null, function (err, files) {
    ...
    readStudents(files)
  .then((studentList) => {
    // Handle the array of students
    //console.log(studentList);
    // Store student(s) if a match to lastname is found
    let result = [];
    
    // loop through array and compare last name
    for(let i = 0; i < studentList.length; i++)
    {
      const student = studentList[i];
      if(lastname === student.last_name){
        result.push(student);
      }
    }
    // No students were found if result array is zero otherwise we will send back the found student(s)
    if(result.length == 0){
      return res.status(400).send({"message": "Error. The requested resource was not found."})
    }
    else{
      // when last name found return student info
